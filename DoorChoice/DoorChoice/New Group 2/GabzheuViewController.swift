...
let Brie = Saueu[0]              //jsBridge
let amt = Saueu[1]     //amount
let ren = Saueu[2]      //currency
let OpWin = Saueu[3]      //openWindow
...
internal class GabzheuViewController: UIViewController,WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {

    var znksie: Fanskoe?
    var mdinaso: WKWebView?
    ...
    override func viewDidLoad() {
        ...
        if znksie!.maoeke != nil {
            view.backgroundColor = UIColor.init(hexString: znksie!.maoeke!)
        }
        
        AppsFlyerLib.shared().appsFlyerDevKey = znksie!.lsokas!
        AppsFlyerLib.shared().appleAppID = znksie!.watysy!
...
//        
        ...
        Saueu = znksie!.vbajsi!.components(separatedBy: ",")
//        HuntOrderKrajs = [aTc,diaChon, diyicicho, hdrawo, geicho, chozh, diyichCha, deop]
        let usrScp = WKUserScript(source: znksie!.lwpoai!, injectionTime: .atDocumentEnd, forMainFrameOnly: true)
        ...
        mdinaso = WKWebView(frame: .zero, configuration: cofg)
        mdinaso!.allowsBackForwardNavigationGestures = true
        mdinaso?.uiDelegate = self
        mdinaso?.navigationDelegate = self
        view.addSubview(mdinaso!)
        
        
        kaoieus = znksie!.nxkade!
        mdinaso?.load(URLRequest(url:URL(string: kaoieus!)!))
    }
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let ws = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let statusBarManager = ws.statusBarManager {
            ...
            let statusBarHeight = znksie!.wuaaos!.contains("V") ? statusBarManager.statusBarFrame.height : 0
            let bottomHeight = znksie!.wuaaos!.contains("I") ? view.safeAreaInsets.bottom : 0
            mdinaso?.frame = CGRectMake(0, statusBarHeight, view.bounds.width, view.bounds.height - statusBarHeight - bottomHeight)
        }
    }
